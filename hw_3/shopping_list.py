def create_shopping_list():
    ...
    if any(shopping_list):
        print(f"\nHere is your shopping list: {shopping_list}")
    else:
    ...
    while True:
        action_input = input("\nTo ADD the product to the list: just enter the product name (ex., milk)\n"
                             "To REMOVE the product from the list: enter \'-\' before the name (ex., -milk):\n").strip()
        ...
        if action_input.startswith('-'):
            ...
            print(f"\n{action_input} is added to the shopping list.")

        # Step 7: Print the updated list of products
        print(f"\nHere is the updated shopping list: {shopping_list}")

        # Step 8: Check if the list is empty using len
        if len(shopping_list) == 0:
            print("\nThe shopping list is empty. That's all for today!")
            break
